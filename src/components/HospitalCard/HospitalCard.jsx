    ...
    <div className={styles.mainCardDiv}>
      <div
        className={`${styles.mainCard} ${
          bookingClick ? styles.openBooking : ""
        }`}
      >
            ...
            <div className={styles.dateTabsWrapper}>
              ...
              <div className={styles.dateTabs}>
                {Array.from({ length: 42 })
                  .slice(startIndex, startIndex + 42)
                  .map((_, i) => {
                    const date = new Date();
                    date.setDate(date.getDate() + (i + startIndex)); // full calendar starting from today + startIndex
                    const dateKey = date.toISOString().split("T")[0];

                    // Label "Today" and "Tomorrow"
                    let label = `${date.getDate()} ${date.toLocaleString(
                      "en-US",
                      { month: "short" }
                    )}`;
                    if (i + startIndex === 0) label = "Today";
                    else if (i + startIndex === 1) label = "Tomorrow";
                ...
                <div
                  ...
                  style={{
                    transform: `translateX(${
                      Array.from({ length: 42 }).findIndex((_, i) => {
                        const d = new Date();
